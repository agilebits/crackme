...
2. Only include cases whose options are available to users today
2. Calculate costs
...
I want to produce a table for an internal survay

```{r survay}
costs_small %>%
  select(method, sample) %>%
  arrange(sample) %>%
  kable(col.names = c("Generation scheme", "Example")) %>%
