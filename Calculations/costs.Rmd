...
It would be nice to translate things like `type:"char", units:8, allow.lcase:TRUE, ...` into something like "8 character password with lowercase, digits" for friendlier display. So we create this ugly scaler function which will happy to be wrapped in an `mapply()` later.
...
1. Only include cases whose strength (in bits) is between 40 and 72.
2. Calculate costs
...
```{r cost-comp}
costs_small <- pwd %>% 
  filter(between(bits, 40, 72)) %>%
  mutate(cost = bits2cost(bits)) %>%
  mutate(pretty = prettyCost(cost)) %>%
  mutate(method = mapply(method_words, type, length, separator, allow.ucase, allow.lcase, allow.digit)) %>%
